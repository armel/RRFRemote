...
void viewMenu(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  ...
  M5.Displays(display).setTextPadding(w - 2);
  M5.Displays(display).setTextColor(TFT_MENU_SELECT);
  M5.Displays(display).drawString(String(NAME) + " V" + String(VERSION) + " by " + String(AUTHOR), 160 + offsetX, 28 + y);

  M5.Displays(display).drawFastHLine(120 + offsetX, 3 + offsetY, 80, TFT_BACK);
