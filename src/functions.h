...
void resetColor() {
  switch (colorCurrent) {
    ...
    case 4:
      TFT_HEADER =
        M5.Displays(display).color565(TFT_HEADER_TURQUOISE.r, TFT_HEADER_TURQUOISE.g, TFT_HEADER_TURQUOISE.b);
    ...
    case 9:
      TFT_HEADER =
        M5.Displays(display).color565(TFT_HEADER_CREPUSCULE.r, TFT_HEADER_CREPUSCULE.g, TFT_HEADER_CREPUSCULE.b);
...
void scroll(uint8_t pause) {
  ...
  while (h--) Sprite.drawFastHLine(0, h, w, TFT_INFO);
...
int computeDistance(float latitudeLink, float longitudeLink) {
  ...
  a = 0.5 - cos((latitudeUser - latitudeLink) * p) / 2.0 +
      cos(latitudeLink * p) * cos(latitudeUser * p) * (1 - cos((longitudeUser - longitudeLink) * p)) / 2.0;
...
bool M5Screen4bmp() {
  ...
  unsigned char header[54] = {'B',     'M',         // BMP signature (Windows 3.1x, 95, NT, …)
  ...
  unsigned char palette[28] = {0,          0,          0,          255, 255,          255,          255,          255,
                               192,        192,        192,        255, BMP_HEADER.b, BMP_HEADER.g, BMP_HEADER.r, 255,
                               BMP_INFO.b, BMP_INFO.g, BMP_INFO.r, 255, BMP_FRONT.b,  BMP_FRONT.g,  BMP_FRONT.r,  255,
  ...
  for (uint16_t y = image_height; y > 0; y -= 10) {
    ...
    for (uint8_t z = 0; z < 10; z++) {
      ...
      for (uint16_t x = 0; x < image_width; x += 2) {
        // Left
        if (line_data_raw[x * 3] == 0 && line_data_raw[x * 3 + 1] == 0 && line_data_raw[x * 3 + 2] == 0) {
          l = 0b0000;
        } else if (abs(line_data_raw[x * 3] - 255) < 16 && abs(line_data_raw[x * 3 + 1] - 255) < 16 &&
                   abs(line_data_raw[x * 3 + 2] - 255) < 16) {
          l = 0b0001;
        } else if (abs(line_data_raw[x * 3] - 192) < 16 && abs(line_data_raw[x * 3 + 1] - 192) < 16 &&
                   abs(line_data_raw[x * 3 + 2] - 192) < 16) {
          l = 0b0010;
        } else if (abs(line_data_raw[x * 3] - BMP_HEADER.r) < 16 && abs(line_data_raw[x * 3 + 1] - BMP_HEADER.g) < 16 &&
                   abs(line_data_raw[x * 3 + 2] - BMP_HEADER.b) < 16) {
          l = 0b0011;
        } else if (abs(line_data_raw[x * 3] - BMP_INFO.r) < 16 && abs(line_data_raw[x * 3 + 1] - BMP_INFO.g) < 16 &&
                   abs(line_data_raw[x * 3 + 2] - BMP_INFO.b) < 16) {
          l = 0b0100;
        } else if (abs(line_data_raw[x * 3] - BMP_FRONT.r) < 16 && abs(line_data_raw[x * 3 + 1] - BMP_FRONT.g) < 16 &&
        ...
        // Right
        if (line_data_raw[(x + 1) * 3] == 0 && line_data_raw[(x + 1) * 3 + 1] == 0 &&
            line_data_raw[(x + 1) * 3 + 2] == 0) {
          r = 0b0000;
        } else if (abs(line_data_raw[(x + 1) * 3] - 255) < 16 && abs(line_data_raw[(x + 1) * 3 + 1] - 255) < 16 &&
                   abs(line_data_raw[(x + 1) * 3 + 2] - 255) < 16) {
          r = 0b0001;
        } else if (abs(line_data_raw[(x + 1) * 3] - 192) < 16 && abs(line_data_raw[(x + 1) * 3 + 1] - 192) < 16 &&
                   abs(line_data_raw[(x + 1) * 3 + 2] - 192) < 16) {
          r = 0b0010;
        } else if (abs(line_data_raw[(x + 1) * 3] - BMP_HEADER.r) < 16 &&
                   abs(line_data_raw[(x + 1) * 3 + 1] - BMP_HEADER.g) < 16 &&
                   abs(line_data_raw[(x + 1) * 3 + 2] - BMP_HEADER.b) < 16) {
          r = 0b0011;
        } else if (abs(line_data_raw[(x + 1) * 3] - BMP_INFO.r) < 16 &&
                   abs(line_data_raw[(x + 1) * 3 + 1] - BMP_INFO.g) < 16 &&
                   abs(line_data_raw[(x + 1) * 3 + 2] - BMP_INFO.b) < 16) {
          r = 0b0100;
        } else if (abs(line_data_raw[(x + 1) * 3] - BMP_FRONT.r) < 16 &&
